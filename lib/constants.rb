module Constants
    FIRST_NAMES = ["Shawn", "Dameon", "Clara", "Ruby", "Joseph", "Darren"]
    LAST_NAMES = ["Stein", "Lloyd", "Fury", "Campbell", "Fisher", "Casperson"]
    COUNTRIES = ["Australia", "United States", "Canada", "China", "Russia", "Italy"]
    STATES = ["Victoria", "Washington", "Montreal", "Wuhan", "Moscow", "Sicily"]
    STREET_ADDRESSES = [
        "100 Smith Street, Smithville",
        "200 John Street, Johnville",
        "300 Anns Way, Mt. Ann",
        "400 Bank Court, Financeville",
        "500 White Road, Plain",
        "600 Redders Lane, Tommatina"
    ]
    N = 6
    # Item constants
    PART_TYPES = ["CPU", "GPU", "RAM", "Motherboard", "Power Supply", "HDD", "SSD"]
    CONDITIONS = ["Mint Condition", "Near New Condition", "Great Condition", "Refurbished", "Working Condition", "Okay Condition", "Not Working"]
    PARTS = [
        [	'	Intel Core i9-10900K Processor	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-10900KF Processor	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-10850K Processor	'	,	'	CPU	'	]	,
        [	'	AMD Ryzen 9 5900X	'	,	'	CPU	'	]	,
        [	'	AMD Ryzen 9 3950X	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-9960X Processor	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-10900 Processor	'	,	'	CPU	'	]	,
        [	'	AMD Ryzen Threadripper 3960X	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-10980XE Extreme Edition Processor	'	,	'	CPU	'	]	,
        [	'	AMD Ryzen 9 3900XT	'	,	'	CPU	'	]	,
        [	'	AMD Ryzen 9 3900X	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-10900F Processor	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-10940X X-series Processor	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-7980XE Processor	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-9900KS Processor	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-9980XE Processor	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-10920X X-series Processor	'	,	'	CPU	'	]	,
        [	'	AMD Ryzen 7 5800X	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-9940X Processor	'	,	'	CPU	'	]	,
        [	'	Intel Core i9-7960X Processor	'	,	'	CPU	'	]	,
        [	'	Nvidia GeForce RTX 3090	'	,	'	GPU	'	]	,
        [	'	AMD Radeon RX 6900 XT	'	,	'	GPU	'	]	,
        [	'	AMD Radeon RX 6800 XT	'	,	'	GPU	'	]	,
        [	'	Nvidia GeForce RTX 3080	'	,	'	GPU	'	]	,
        [	'	AMD Radeon RX 6800	'	,	'	GPU	'	]	,
        [	'	Nvidia Titan RTX	'	,	'	GPU	'	]	,
        [	'	Nvidia GeForce RTX 2080 Ti	'	,	'	GPU	'	]	,
        [	'	Nvidia GeForce RTX 3070	'	,	'	GPU	'	]	,
        [	'	AMD Radeon RX 6700 XT	'	,	'	GPU	'	]	,
        [	'	Nvidia GeForce RTX 3060 Ti	'	,	'	GPU	'	]	,
        [	'	Nvidia Titan V	'	,	'	GPU	'	]	,
        [	'	Nvidia GeForce RTX 2080 Super	'	,	'	GPU	'	]	,
        [	'	Nvidia GeForce RTX 2080	'	,	'	GPU	'	]	,
        [	'	Nvidia Titan Xp	'	,	'	GPU	'	]	,
        [	'	Nvidia GeForce RTX 2070 Super	'	,	'	GPU	'	]	,
        [	'	AMD Radeon VII	'	,	'	GPU	'	]	,
        [	'	Nvidia GeForce GTX 1080 Ti	'	,	'	GPU	'	]	,
        [	'	AMD Radeon RX 5700 XT	'	,	'	GPU	'	]	,
        [	'	Nvidia GeForce RTX 3060 12GB	'	,	'	GPU	'	]	,
        [	'	Nvidia GeForce RTX 2070	'	,	'	GPU	'	]	,
        [	'	TeamGroup T-Force Xtreem ARGB (2x 8GB)	'	,	'	RAM	'	]	,
        [	'	Corsair Dominator Platinum RGB 32GB	'	,	'	RAM	'	]	,
        [	'	G.Skill Trident Z Neo DDR4-3600 (4x8GB)	'	,	'	RAM	'	]	,
        [	'	G.Skill Trident Z Royal DDR4-4000 16GB	'	,	'	RAM	'	]	,
        [	'	G.Skill Ripjaws V 16GB (2 x 8GB)	'	,	'	RAM	'	]	
    ]
end